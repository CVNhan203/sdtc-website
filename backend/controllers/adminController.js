...
// Lấy danh sách staff đã xóa
exports.getDeletedStaffs = asyncHandler(async (req, res) => {
  const deletedStaffs = await Admin.find({ 
    role: 'staff', 
    isDeleted: true  
  }).select('-password');
...
// Khôi phục staff đã xóa
exports.restoreStaff = asyncHandler(async (req, res) => {
  const { id } = req.params;
  const staff = await Admin.findOneAndUpdate(
    { _id: id, role: 'staff', isDeleted: true },
    { $set: { isDeleted: false } },
    { new: true }
  );
  ...
  if (!staff) {
    return res.status(404).json({ 
      success: false, 
      message: 'Không tìm thấy staff đã xóa' 
    });
  }
  
  res.json({ 
    success: true, 
    message: 'Đã khôi phục staff thành công',
    data: staff 
  });
});

// Xóa vĩnh viễn staff
exports.permanentDeleteStaff = asyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  res.json({ 
    success: true, 
    message: 'Đã xóa vĩnh viễn staff thành công' 
  });
});
