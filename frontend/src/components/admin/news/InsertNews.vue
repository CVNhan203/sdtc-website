      ...
      <form @submit.prevent="handleSubmit" :class="{ 'submitting': isSubmitting }">
        ...
        <div class="form-actions">
          ...
          >
            ...
            {{ isSubmitting ? 'Đang lưu...' : 'Thêm tin tức' }}
          </button>
        </div>
      </form>
    </div>
  ...
  setup() {
    ...
    const hasChanges = computed(() => {
      return Object.keys(formData.value).some(key => {
        if (key === 'image') {
          return formData.value[key] !== initialFormData[key]
        }
        return formData.value[key]?.toString() !== initialFormData[key]?.toString()
      })
    })
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        if (response.data.success) {
          ...
          throw new Error(response.data.message || 'Có lỗi xảy ra khi tạo tin tức')
        }
      } catch (error) {
        console.error('Error creating news:', error)
        eventBus.emit('show-toast', {
          type: 'error',
          message: error.response?.data?.message || 'Có lỗi xảy ra khi tạo tin tức. Vui lòng thử lại.'
        })
      } finally {
