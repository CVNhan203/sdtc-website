...
import api from '../config' // Nhập cấu hình API từ file config

const serviceService = {
  // Lấy danh sách dịch vụ
  async getServices(params) {
    try {
      const response = await api.get('/services', { params }) // Gọi API để lấy danh sách dịch vụ
      return {
  ...
  async createService(serviceData) {
    try {
      ...
      // Gán loại dịch vụ và tiêu đề
      serviceData.type = serviceData.type || 'web';
      serviceData.title = serviceData.title?.trim() || 'Dịch vụ mới ' + new Date().toLocaleDateString();

      console.log('Sending service data to backend:', serviceData); // Ghi log dữ liệu dịch vụ
      const response = await api.post('/services', serviceData); // Gọi API để tạo dịch vụ
      console.log('Service created successfully:', response.data); // Ghi log thông báo thành công
